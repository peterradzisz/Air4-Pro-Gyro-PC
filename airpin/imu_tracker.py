...
class ImuTracker:
    def __init__(self):
        ...
        self.gyro_deadzone = GYRO_DEADZONE
    ...
    def _poll_loop(self):
        ...
        try:
            while self._running:
                ...
                gc = np.where(np.abs(gc) > self.gyro_deadzone, gc, 0.0)
                ...
                # Continuous bias estimation (ZUPT)
                raw_mag = float(np.sqrt(np.sum(gyro * gyro)))
                if raw_mag < STILL_THRESHOLD:
                    self._still_counter += 1
                    if self._still_counter >= STILL_SAMPLES:
                        self._gyro_bias = self._gyro_bias * (1.0 - BIAS_LEARN_RATE) + gyro * BIAS_LEARN_RATE
                else:
                    self._still_counter = 0
